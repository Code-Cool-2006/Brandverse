...
const StaffDashboard = () => {
  ...
  const [form, setForm] = useState({ name: "", type: "restaurant", category: "multi", image: "" });
  const [error, setError] = useState("");
  const [selectedRestaurantId, setSelectedRestaurantId] = useState(null);
  const [selectedRestaurantName, setSelectedRestaurantName] = useState("");
  const [dishesByRestaurant, setDishesByRestaurant] = useState({});
  const [dishForm, setDishForm] = useState({ name: "", price: "", image: "", category: "veg", diabeticFriendly: false });
  ...
  const fetchDishes = async (restaurantId) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/restaurants/${restaurantId}/dishes`);
      if (!res.ok) {
      ...
      const data = contentType.includes('application/json') ? await res.json() : { dishes: [] };
      setDishesByRestaurant((prev) => ({ ...prev, [restaurantId]: data.dishes || [] }));
    } catch (e) {
      console.error(e);
      setError(`Failed to load dishes for restaurant ${restaurantId}. Is the server running on http://localhost:5000?`);
    } finally {
  ...
  const handleCreateDish = async (e) => {
    ...
    if (!selectedRestaurantId) {
      setError("Please select a restaurant from the list above before adding a dish.");
      return;
    }
    // Resolve the current restaurant; if ID is stale (server restarted), try by name after a refresh
    let current = restaurants.find(r => r.id === selectedRestaurantId) || (selectedRestaurantName ? restaurants.find(r => r.name === selectedRestaurantName) : null);
    if (!current) {
      const freshList = await fetchRestaurants();
      current = freshList.find(r => r.id === selectedRestaurantId) || (selectedRestaurantName ? freshList.find(r => r.name === selectedRestaurantName) : null);
      if (!current) {
        setSelectedRestaurantId(null);
        setSelectedRestaurantName("");
        setError("Selected restaurant no longer exists on the server. Please reselect a restaurant and try again.");
        return;
      }
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/restaurants/${current.id}/dishes`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (res.status === 404) {
            message = "Restaurant not found on server. Try refreshing the restaurants list or re-create the restaurant (server may have restarted).";
          } else if (!text) {
      ...
      setDishForm({ name: "", price: "", image: "", category: "veg", diabeticFriendly: false });
  ...
  const fetchRestaurants = async () => {
    try {
      ...
      const contentType = res.headers.get('content-type') || '';
      const data = contentType.includes('application/json') ? await res.json() : { restaurants: [] };
      const list = Array.isArray(data.restaurants) ? data.restaurants : [];
      setRestaurants(list);
      if (selectedRestaurantId && !list.some(r => r.id === selectedRestaurantId)) {
              ...
              <div className="management-grid restaurants-grid">
                <div className="management-card" style={{ gridColumn: "1 / -1" }}>
                  ...
                  <form className="restaurant-form" onSubmit={handleCreateRestaurant}>
                    ...
                    <button className="manage-btn" type="submit" disabled={creating}>
                ...
                <div className="management-card" style={{ gridColumn: "1 / -1" }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button className="manage-btn" style={{ maxWidth: 140 }} onClick={fetchRestaurants}>Refresh</button>
                  </div>
                  <p>Manage restaurant partnerships</p>
                  <div className="restaurants-list">
                    {restaurants.map((r) => (
                      <div key={r.id} className={`restaurant-row ${selectedRestaurantId === r.id ? 'selected' : ''}`} onClick={() => handleSelectRestaurant(r.id)} style={{ cursor: 'pointer' }}>
                        <img src={r.image || "https://via.placeholder.com/60x40?text=Img"} alt={r.name} />
                        <div className="restaurant-info">
                          <div className="restaurant-name">{r.name}</div>
                          <div className="restaurant-meta">{r.type} • {r.category}</div>
                        </div>
                      </div>
                    ))}
                    {restaurants.length === 0 && <div>No restaurants yet. Add your first one above.</div>}
                  </div>
                </div>
                ...
                {selectedRestaurantId && (
                  <div className="management-card" style={{ gridColumn: "1 / -1" }}>
                    <h3>Manage Dishes</h3>
                    <p style={{ fontWeight: 600 }}>Selected: {restaurants.find(r => r.id === selectedRestaurantId)?.name || selectedRestaurantName || (selectedRestaurantId ? `#${selectedRestaurantId}` : 'None')}</p>
                    <p>Add dishes for the selected restaurant</p>
                    {error && <div className="error-banner">{error}</div>}
                    <form className="restaurant-form" onSubmit={handleCreateDish}>
                      <div className="form-row">
                        <label>Dish Name</label>
                        <input type="text" value={dishForm.name} onChange={(e) => setDishForm({ ...dishForm, name: e.target.value })} required />
                      </div>
                      <div className="form-row">
                        <label>Price</label>
                        <input type="number" step="0.01" value={dishForm.price} onChange={(e) => setDishForm({ ...dishForm, price: e.target.value })} required />
                      </div>
                      <div className="form-row">
                        <label>Image URL</label>
                        <input type="url" value={dishForm.image} onChange={(e) => setDishForm({ ...dishForm, image: e.target.value })} placeholder="https://..." />
                      </div>
                      <div className="form-row">
                        <label>Category</label>
                        <input type="text" value={dishForm.category} onChange={(e) => setDishForm({ ...dishForm, category: e.target.value })} placeholder="veg | non-veg | multi" />
                      </div>
                      <div className="form-row" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <input id="diabeticFriendly" type="checkbox" checked={dishForm.diabeticFriendly} onChange={(e) => setDishForm({ ...dishForm, diabeticFriendly: e.target.checked })} />
                        <label htmlFor="diabeticFriendly">Diabetes-friendly</label>
                      </div>
                      <button className="manage-btn" type="submit" disabled={creatingDish}>{creatingDish ? 'Adding...' : 'Add Dish'}</button>
                    </form>

                    <h4 style={{ marginTop: '20px' }}>Existing Dishes</h4>
                    <div className="restaurants-list">
                      {(dishesByRestaurant[selectedRestaurantId] || []).map((d) => (
                        <div key={d.id} className="restaurant-row">
                          <img src={d.image || "https://via.placeholder.com/60x40?text=Img"} alt={d.name} />
                          <div className="restaurant-info">
                            <div className="restaurant-name">{d.name} - ${d.price.toFixed(2)}</div>
                            <div className="restaurant-meta">{d.category}{d.diabeticFriendly ? ' • diabetes-friendly' : ''}</div>
                          </div>
                        </div>
                      ))}
                      {(dishesByRestaurant[selectedRestaurantId] || []).length === 0 && <div>No dishes yet.</div>}
                    </div>
                  </div>
                )}
              </div>
